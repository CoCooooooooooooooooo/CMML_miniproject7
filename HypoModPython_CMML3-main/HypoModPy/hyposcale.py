...
class ScaleBox(ToolPanel):
    def __init__(self, parent, size, numdraw):
        ...
        for graphpanel in self.mainwin.panelset:
            ...
            if panelindex == 0:  # Change: show Overlay button on panel 1
                DiagWrite("ScaleBox panel 0\n")
                ...
                self.overset[ID_Overlay] = OverDat(ID_Overlay, 0, 1)  # Change: merge curves from panel 0 and 1

            if panelindex == 2:  # Add: show Overlay button on panel 3
                DiagWrite("ScaleBox panel 2\n")
                hbox = wx.BoxSizer(wx.HORIZONTAL)
                self.ScaleButton(ID_Overlay3, "Overlay", 48, hbox).Bind(wx.EVT_BUTTON, self.OnOverlay)
                graphpanel.consolebox.Add(hbox, 0, wx.ALIGN_CENTRE_HORIZONTAL|wx.ALIGN_CENTRE_VERTICAL|wx.ALL, 2)
                self.overset[ID_Overlay3] = OverDat(ID_Overlay3, 2, 3)  # Merge curves from panel 2 and 3
    ...
    def GetPanelByIndex(self, index):
        for panel in self.mainwin.panelset:
    ...
    def OnYZoomIn(self, event):
        ...
        graphpanel = self.GetPanelByIndex(index)
        if graphpanel is None or len(graphpanel.dispset) == 0: return
        plot = graphpanel.GetFrontPlot()
