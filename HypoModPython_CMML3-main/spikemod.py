...
class SpikeMod(Mod):
    ...
    def PlotData(self):
        ...
        # Overlay hist 5ms norm
        self.plotbase.NewSet("Overlay Hist 5ms Norm", "overlayhist5norm")
        self.plotbase.AddPlot(PlotDat(self.cellspike.hist5norm, 0, 2000, 0, 500, "Cell Hist 5ms Norm", "line", 1, "blue"), "overlaycellhist5norm", "overlayhist5norm")
        self.plotbase.AddPlot(PlotDat(self.modspike.hist5norm, 0, 2000, 0, 500, "Mod Hist 5ms Norm", "line", 1, "green"), "overlaymodhist5norm", "overlayhist5norm")
        ...
        # Overlay haz 5ms
        self.plotbase.NewSet("Overlay Haz 5ms", "overlayhaz5")
        self.plotbase.AddPlot(PlotDat(self.cellspike.haz5, 0, 2000, 0, 100, "Cell Haz 5ms", "line", 1, "blue"), "overlaycellhaz5", "overlayhaz5")
        self.plotbase.AddPlot(PlotDat(self.modspike.haz5, 0, 2000, 0, 100, "Mod Haz 5ms", "line", 1, "green"), "overlaymodhaz5", "overlayhaz5")
    ...
    def DefaultPlots(self):
        if len(self.mainwin.panelset) > 0: self.mainwin.panelset[0].settag = "overlayhist5norm"
        if len(self.mainwin.panelset) > 1: self.mainwin.panelset[1].settag = "modhist5norm"
        if len(self.mainwin.panelset) > 2: self.mainwin.panelset[2].settag = "overlayhaz5"
        if len(self.mainwin.panelset) > 3: self.mainwin.panelset[3].settag = "modhaz5"
    ...
    def NeuroData(self):
        ...
        # convert loaded cell spike times from seconds to milliseconds if needed
        celldata = self.spikedata[self.cellindex]
        if celldata.spikecount > 0 and celldata.times[0] < 10:
            DiagWrite("Converting cell spike times from s to ms\n")
            celldata.times[:celldata.spikecount] *= 1000

        self.cellspike.Analysis(celldata)
        self.cellspike.id = self.cellindex
        self.cellspike.name = celldata.name
    ...
    def OnModThreadComplete(self, event):
        ...
        self.runflag = False
        self.ModelData()
        print(f"Run complete. Spikes: {self.modspike.spikecount}, Freq: {self.modspike.freq:.2f} Hz")
        print(f"To export: from export_data import export_data; export_data(mod, label='your_label')")
...
class SpikeModel(ModThread):
    ...
    def Model(self):
        ...
        if halflifeDAP <= 0:
            halflifeDAP = 8.0  # default value
        kAHP = spikeparams["kAHP"]
        halflifeAHP = spikeparams["halflifeAHP"]
        if halflifeAHP <= 0:
            halflifeAHP = 500.0

        # DAP model selection: 0=none, 1=simple, 2=NMDA-based
        DAP_model = int(spikeparams["DAP_model"])  # 0=no DAP, 1=simple DAP, 2=NMDA DAP
        
        # NMDA-based DAP parameters (only used if DAP_model==2)
        gNMDA = spikeparams["gNMDA"]  # conductance (mS/cm2)
        tau_NMDA_rise = spikeparams["tau_NMDA_rise"]  # ms
        tau_NMDA_decay = spikeparams["tau_NMDA_decay"]  # ms
        tau_x_NMDA = spikeparams["tau_x_NMDA"]  # ms
        Mg_conc = spikeparams["Mg_conc"]  # mM (extracellular)
        NMDA_Ca_fraction = spikeparams["NMDA_Ca_fraction"]
        E_NMDA = spikeparams["E_NMDA"]  # mV
        ...
        # NMDA-based DAP time constants
        alpha_NMDA = 1.0 / tau_NMDA_rise  # opening rate
        beta_NMDA = 1.0 / tau_NMDA_decay  # closing rate
        ...
        # NMDA-based DAP variables
        s_NMDA = 0.0  # gating variable (0 to 1, fast component)
        x_NMDA = 0.0  # synaptic glutamate variable (marker of PSP activity)
        I_NMDA = 0.0  # NMDA current
        V_NMDA = 0.0  # NMDA-mediated depolarization contribution
        ...
        for i in range(1, runtime + 1):
            ...
            # DAP dynamics (depends on selected model)
            if DAP_model == 1:
                # Simple DAP (traditional kDAP-based)
                tDAP = tDAP - tDAP * tauDAP
            elif DAP_model == 2:
                # NMDA-based DAP dynamics
                # PSP-driven glutamate release dynamics
                # First decay, then add new input
                x_NMDA *= math.exp(-hstep / tau_x_NMDA)
                if nepsp > 0:
                    x_NMDA = min(1.0, x_NMDA + nepsp * 0.02)  # glutamate increases
                
                # NMDA receptor gating: two-state model
                s_NMDA += (alpha_NMDA * x_NMDA * (1 - s_NMDA) - beta_NMDA * s_NMDA) * hstep
                
                # Voltage-dependent Mg2+ block (Woodhull model, Jahr & Stevens 1990)
                Mg_block = 1.0 / (1.0 + (Mg_conc / 3.57) * math.exp(-0.062 * V))
                
                # NMDA current
                driving_force = V - E_NMDA
                I_NMDA = gNMDA * s_NMDA * Mg_block * driving_force
                NMDA_scale = 5.0  # scaling factor for voltage contribution
                V_NMDA = -I_NMDA * NMDA_scale
                
                # Ca2+ contribution (only for inward current)
                if I_NMDA < 0:  # inward current (negative)
                    Ca_influx = -NMDA_Ca_fraction * I_NMDA * 0.001
                    tE = tE + Ca_influx
            # else: DAP_model == 0, no DAP (neither tDAP nor V_NMDA)
            
            # Membrane potential calculation based on DAP model
            if DAP_model == 1:
                # Simple DAP model
                V = Vrest + tPSP + tDAP - tHAP - tAHP
            elif DAP_model == 2:
                # NMDA-based DAP model (replaces kDAP-based DAP)
                V = Vrest + tPSP - tHAP - tAHP + V_NMDA
            else:
                # No DAP model
            ...
            if V > Vthresh and ttime >= absref:
                ...
                # Spike incremented variables - afterpotentials
                tHAP = tHAP + kHAP
                
                # DAP increment depends on model selection
                if DAP_model == 1:
                    # Simple DAP model: spike-driven increment
                    tDAP = tDAP + kDAP
                elif DAP_model == 2:
                    # NMDA-based DAP: partial activation on spike
                    s_NMDA = min(1.0, s_NMDA + 0.02)
                # else: DAP_model==0, no DAP increment
